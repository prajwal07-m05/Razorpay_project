...
// Render Free + Gemini investigations can legitimately take several seconds.
// Keep enough time for a cold backend wake-up and a live AI investigation.
...
// it to distinguish "backend unavailable" from a normal offline degradation
// and to surface an explicit error rather than substituting local data.
...
function offlineOr<T>(fallback: () => T): { value: T; source: Source } {
  ...
  if (REQUIRE_BACKEND) {
    throw new BackendRequired("backend required but unreachable")
  }
...
function offlineRun(mode: Mode): AuditRun {
  ...
  return {
    mode,
    summary: summarize(batch.dataset, cases),
    cases,
  }
}
...
export async function fetchAudit(
  mode: Mode,
): Promise<{ run: AuditRun; source: Source }> {
  try {
    const run = await req<AuditRun>(`/api/audit/run?mode=${mode}`, {
      method: "POST",
    })
    ...
    if (e instanceof BackendRequired) {
      throw e
    }
...
export async function fetchMetrics(
  mode: Mode,
): Promise<{ metrics: Metrics; source: Source }> {
  try {
...
export async function investigate(
  c: Case,
  mode: Mode,
): Promise<{ investigation: Investigation; source: Source }> {
  try {
    const investigation = await req<Investigation>(
      `/api/cases/${c.case_id}/investigate?mode=${mode}`,
      {
        method: "POST",
      },
    )
...
export async function approve(
  c: Case,
): Promise<{ entry: AuditEntry; source: Source }> {
  ...
  try {
    const res = await req<{ audit_entry: AuditEntry }>(
      `/api/cases/${c.case_id}/approve`,
      {
        method: "POST",
        body: JSON.stringify({
          actor: "human@merchant",
    ...
    // REQUIRE_BACKEND on → offlineOr throws BackendRequired: no local approval
    // is fabricated and the caller reports the failure.
    // REQUIRE_BACKEND off → dev-only local draft entry (offline development mode).
