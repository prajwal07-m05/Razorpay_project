...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      strictPort: true,
      watch: { ignored: ['**/.figma/**'] },
      // Same-origin proxy to the FastAPI backend. The browser calls the preview
      // origin at /api/*, and this dev server (which shares the sandbox with the
      // backend) forwards to it — so the preview shows real, authoritative data
      // instead of falling back to the in-browser engine. Override the target
      // with CASEFILE_API_TARGET when the backend runs elsewhere.
      proxy: {
        '/api': {
